 	...
 	<div class="formwin">
        ...
        <el-progress :text-inside="true" :stroke-width="22" :percentage="upload_status" style="width: 60%;margin-left: 20%;margin-top: -35%;"></el-progress>
 			...
 			<div class="content" :style="'height:' + wininfo.contenthei">
			 	...
			 	<div class="formline" style="margin-bottom: 7px;">
...
export default {
  ...
  created () {
    ...
    //如果mock开启了 系统自动刷新 页面并关闭mock
    var moni = JSON.parse(localStorage.getItem("moni"))
    if(moni){
      localStorage.setItem("moni",false)
  ...
  methods: {
    ...
    savedata () {
      console.log('进入保存事件，并开始检测')
      debugger
      if (!this.valid()) {
        return false
      }
      if(this.lessonid==""){
        this.win('请先添加章节，上传资源！','',false);
        retuen;
      }
    ...
    cancelfn () {
      this.$router.push({name: 'AJcourseManageres', query: {courseId: this.courseId}})
    },
    ...
    if (obj) {
      ...
      .then(function (res) {
        ...
        for (let item = 0; item < objs.length; item++) {
          ...
          if (item === 0) {
            that.lessonid = objs[item].lessonId
          }
        }
        if (temp.length <= 0) {
          temp = [{value: '', txt: '请选择'}]
