<template>
 	<div class="formwin">
 		<div class="win_content" :style="'width:' + wininfo.widhei[0] + ';height:' + wininfo.widhei[1]">
 			<div class="title">
 				<span class="menuname">{{evaluationtempletsetinfo.checkeidtN}}</span>
...
export default {
  name: 'AJevaluateTemplateloopChcekedit',
  data () {
    return {
      wininfo: {
        widhei: ['600px', '380px'],
  ...
  mounted () {
    ...
    if (!this.evaluateinfo) {
      ...
      this.num = obj.num
      this.selectval = obj.checkinfo.length
      this.matchval = obj.checkinfo.length
      let temp = []
      for (let item = 0; item < this.selectval; item++) {
        temp.push(obj.checkinfo[item].txt)
      }
      this.radioval = temp
      this.$Select({
        'data': selectinfo,
