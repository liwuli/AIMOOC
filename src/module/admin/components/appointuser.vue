    	...
    	<div class="centers">
    		...
    		<div class="AA_right">
		        ...
		        <div class="appointlist">
		        	<div class="aplist"  v-for="(item, ind) in apinfo">
		        		<span v-if="item.state"  @click='detailevent(item)'   style="background: #4e4e4e;color:#fff">{{item.name}}--{{item.use}}</span>
		        		<span v-if="!item.state"  @click='detailevent(item)'   >{{item.name}}--{{item.nouse}}</span>
		        		<div  v-if="item.state"   @click='detailsevent(item)'  style="color:#fff" class="detail">{{basicinfo.detailN}}</div>
		        		<div  v-if="!item.state"  @click='detailsevent(item)'  class="detail">{{basicinfo.detailN}}</div>
		        	</div>
...
import {basicinfo, timelisturl, findtimesurl, listurl, pathinfo, statearr, typearr, roomurl, pagesize} from '@/module/admin/config/appointuserinfo'
export default {
  name: 'AJappointuser',
  data () {
    return {
  ...
  methods: {
    ...
    detailevent (item) {
      this.$router.push({'name': this.pathinfo.prex + 'add', params: {item: item}})
    },
    detailsevent (item) {
    ...
    pagepost () { // 获取列表
      ...
      .then(function (res) {
        ...
        for (let i = 0; i < objs.length; i++) {
          temp.push({
            dates: that.dateval,
            id: objs[i].classroomId,
            name: objs[i].classroomName,
            state: objs[i].Bookings.length,
            use: that.use,
            nouse: that.nouse,
            controlrecord: objs[i].isForControlRecording,
            dailyrecord: objs[i].isForDailyRecording
          })
        }
        that.apinfo = temp
      })
