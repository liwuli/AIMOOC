 		...
 		<div class="win_content" :style="'width:' + wininfo.widhei[0] + ';height:' + wininfo.widhei[1]">
 			...
 			<div class="content" :style="'height:' + wininfo.contenthei">
			 	...
			 	<div class="formline">
			 		...
			 		<input type="number" min="0" v-model="scole"  :value="scole" :placeholder="evaluationtempletsetinfo.contentscole"/>
			 	</div>
 			</div>
 			<div class="btnline">
...
import {warntxt, saveN, cancelN, evaluationtempletsetinfo, pathinfo, editurl} from '@/module/admin/config/evaluationtemplatesetinfo'
export default {
    ...
    savedata () {
      ...
      .then(function (res) {
        if (!res.data.isSuccess) {
          ...
          that.$router.push({'name': that.pathinfo.prex + 'settemplate', query: {name: that.templatename, templateid: that.evaluateTemplateId}, params: {id: that.parentid, nowscole: res.data.total}})
        }
      })
  ...
  mounted () {
    ...
    if (!obj) {
      ...
      this.name = obj.name
      this.parentid = this.$route.params.parentitem.id
    }
  }
}
</script>
