...
import {delurl, cancelN, delN, evaluationtempletsetinfo, pathinfo} from '@/module/admin/config/evaluationtemplatesetinfo'
export default {
  name: 'AJevaluateTemplatesettemplatecontentdel',
  data () {
    return {
      msg: evaluationtempletsetinfo.delcontent,
      wininfo: {
        widhei: ['300px', '270px'],
        btncancelleft: '145px',
        contenthei: '153px'
    ...
    savedata () {
      ...
      .then(function (res) {
        if (!res.data.isSuccess) {
          ...
          that.$router.push({'name': that.pathinfo.prex + 'settemplate', query: {name: that.templatename, templateid: that.evaluateTemplateId}, params: {id: that.parentid}})
        }
      })
