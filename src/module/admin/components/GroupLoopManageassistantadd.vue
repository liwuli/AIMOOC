...
import {assistantaddmemberurl, getcollegeurl, assistantaddurl, loopclassgroupinfo, relassistantpathinfo, pageval, relassistantcolumsadd, relassistantpercentinfoadd, relassistantlistheadadd, pagesize} from '@/module/admin/config/GroupLoopManageinfo'
export default {
  ...
  methods: {
    ...
    cancelfn () {
      this.$router.push({name: relassistantpathinfo.prex, query: {ClassGroupId: this.loopClassGroupId, groupType: this.grouptype}})
    },
    pagepost (obj) {
      let that = this
      ...
      .then(function (res) {
        let temp = []
        let objs = res.data.classesTeacherList
