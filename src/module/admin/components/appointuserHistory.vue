    	...
    	<div class="centers">
    		...
    		<div class="AA_right">
		        ...
		        <div class="AA_right" style="margin: 15px 0 0 0;">
			      <List :totalnum="totalnum" :delbtn='show' :jumpinfo="jumpinfo" :addbtn='show' :pagesize="pagesizeten" :pageval = "pageval" :bingos="bingos" @pagepost="pagepost" :pageinfo="pageinfo"  :pathinfo="pathinfo" :info='products' :infoval="infoval" :head='listhead' :optevent='optevent' :colums="colums" :percentinfo='percentinfo'></List>
	    		</div>
    		</div>
    	</div>
    </div>
...
import {basicinfo, jumpinfo, getappointlisturl, bingos, pageval, optobjinfoAManageshow, optobjinfoAManage, pathinfo, opteventAManage, percentinfoAManage, columsAManage, pagesizeten, listheadAManage, statearrManage, typearr, roomurl} from '@/module/admin/config/appointuserinfo'
export default {
  ...
  methods: {
    ...
    detailevent (item) {
      this.$router.push({'name': this.jumpinfo.prex + 'jump', params: {item: item}})
    },
    buildevent (item, ind) {
      this.roomselect = []
    ...
    pagepost () { // 获取列表
      ...
      if (this.stateval) {
        dataval.statusList = [this.stateval + 1]
      }
      if (this.typeval) {
  ...
  watch: {
    '$route' (to, from) {
      if (to.name === 'AJappointuserHistory') {
        this.pagepost()
      }
    }
  }
