...
import {basicinfo, warntxt, pathinfo, radioinfo, manufacturerinfo, device, editurl} from '@/module/admin/config/controlrecordinfo'
export default {
  ...
  mounted () {
    ...
    if (!obj) {
      ...
      this.http = obj.protocolType !== null ? obj.protocolType : -1
      if (this.http === 2 || this.http === '2') {
        this.show = false
      } else {
        this.manu = obj.manu.toString()
        this.PORT = obj.port
      }
      this.user = obj.uname
      this.pass = obj.ukeyword
