...
import {basicinfo, powerinfo, pageval, infoval, opteventAll, optobjinfoAll, products, bingos, percentinfo, colums, pagesize, listurl, listhead, pathinfo} from '@/module/admin/config/subjectinfo'
// 权限处理
  ...
  methods: {
    pagepost (obj) {
      ...
      if (this.$store.state.userinfo.userName === 'admin') {
        ...
        let power = this.$store.state.powerinfo['subject']
        if (power.length !== 0) {
      ...
      .then(function (res) {
        ...
        for (let item = start; item < (that.pageval * pagesize < objs.length ? that.pageval * pagesize : objs.length); item++) {
          ...
          temp[item - start]['subjectName'] = objs[item].subjectName
