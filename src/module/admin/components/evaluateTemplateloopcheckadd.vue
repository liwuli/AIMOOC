<template>
 	<div class="formwin">
 		<div class="win_content" :style="'width:' + wininfo.widhei[0] + ';height:' + wininfo.widhei[1]">
 			<div class="title">
 				<span class="menuname">{{evaluationtempletsetinfo.checkaddN}}</span>
...
export default {
  name: 'AJevaluateTemplateloopCheckadd',
  data () {
    return {
      wininfo: {
        widhei: ['600px', '380px'],
  ...
  methods: {
    ...
    savedata () {
      if (!this.valid()) {
        ...
        let tempradioinfo = {}
        tempradioinfo.num = this.evaluateinfo.length
        tempradioinfo.type = 'check'
        tempradioinfo.aetype = 'add'
        tempradioinfo.title = this.title
        tempradioinfo.display = true
        tempradioinfo.checkinfo = []
        for (let item = 0; item < this.radioval.length; item++) {
          tempradioinfo.checkinfo.push({txt: this.radioval[item], val: item})
        }
        this.$router.push({name: pathinfo.prex, params: {radioinfo: tempradioinfo}, query: {templateid: this.templateid}})
      }
    },
    cancelfn () {
