...
import {basicinfo, warntxt, pathinfo, editurl} from '@/module/admin/config/buildinginfo'
export default {
  name: 'AJbuildingedit',
  data () {
    return {
      msg: basicinfo.msgroomeditN,
      basicinfo: basicinfo,
      wininfo: {
        widhei: ['500px', '370px'],
        btncancelleft: '345px',
      ...
      if (this.valid()) {
        ...
        this.axios({
          ...
          params: {
            'params': {
              buildingId: that.id,
              buildingDescription: that.description,
              buildingName: that.name
            }
          }
        })
        .then(function (res) {
