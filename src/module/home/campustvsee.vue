...
//    }
    ...
    getsearchval () {
      ...
      .then(function (res) {
        ...
        if (objs.tvProgramStatus !== 2) {
          ...
          that.$router.push({name: 'Fcampustvsee', query: {channelid: that.channelid, dateval: that.dateval, programid: objs.tvProgramId}})
          that.$Video({
    ...
    channelfn (item, ind) {
      ...
      // 执行查询接口
//    this.$router.push({name: 'Fcampustvsee', query: {channelid: this.channelid, dateval: this.dateval, programid: this.programid}})
//    this.$router.go(0)
      this.getprogram('channelchange')
    },
    leftright (item) {
      let minlen = 6
      let minwid = 180
