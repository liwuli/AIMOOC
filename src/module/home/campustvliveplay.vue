...
import {getservertimeurl, programseelisturl, channellisturl, programlisturl, searchurl, tabinfo, tabselected, liststate, campustvinfo} from '@/module/home/config/campustvinfo'
export default {
  ...
  methods: {
    getservertime () {
      ...
      .then(function (res) {
        ...
        that.nowtime = that.servertime = new Date(time).getTime()
    ...
    emptydom () {
      if (this.vidoeobj) {
        ...
        this.vidoeobj.cfg.V_pause.click()
      }
//    if (this.intervalcount) {
//      clearInterval(this.intervalcount)
//    }
      if (document.getElementById('playerArealocal')) {
        document.getElementById('playerArealocal').innerHTML = ''
      }
    },
    jumpto (item) {

        // 这里修改了 逻辑 只有点会看的时候才会暂停
      switch (item.btn) {
        case 'btnred': // 直播中
          if (item.programType === 0) { // 直播
            this.$router.push({name: 'Fcampustvlive', query: {channelid: item.channelid, dateval: this.dateval, programid: item.id}})
          } else {
            this.$router.push({name: 'Fcampustvliveplay', query: {channelid: item.channelid, dateval: this.dateval, programid: item.id}})
        ...
        case 'btnblue': // 回看
          this.emptydom()  //暂停
          this.$router.push({name: 'Fcampustvsee', query: {channelid: item.channelid, dateval: this.dateval, programid: item.id}})
          break
      }
    },
    getsearchval () {
      ...
      .then(function (res) {
        ...
        if (objs.programType === 0) {
          ...
          that.starttime = new Date(objs.dayTime + ' ' + objs.startTime).getTime()
          let jumptotime = (new Date(that.nowtime).getTime() - that.starttime) / 1000
          that.vidoeobj = that.$Video({
            channelB: [objs.videoUrl],
    ...
    channelfn (item, ind) {
      ...
      // 执行查询接口
      this.$router.push({name: 'Fcampustvliveplay', query: {channelid: this.channelid, dateval: this.dateval, programid: this.programid}})
      this.emptydom()
      this.getprogram()
//    this.searchval()
    },
